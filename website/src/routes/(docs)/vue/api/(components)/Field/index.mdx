---
title: Field
description: Headless form field component that provides reactive properties and state.
source: /frameworks/vue/src/components/Field/Field.vue
contributors:
  - fabian-hiller
---

import { Link } from '@qwik.dev/router';
import { ApiList, Property } from '~/components';
import { properties } from './properties';

# Field

Headless form field component that provides reactive properties and state. The field component takes a form store, path to field, and a scoped slot that receives a field store to display field state and handle user interactions.

```vue
<Field of="{…}" path="{…}" v-slot="field">
  <!-- Use field store here -->
</Field>
```

## Generics

- `TSchema` <Property {...properties.TSchema} />
- `TFieldPath` <Property {...properties.TFieldPath} />

## Properties

- `of` <Property {...properties.of} />
- `path` <Property {...properties.path} />

## Slots

- `default` - Scoped slot that receives a <Link href="../FieldStore/">`FieldStore`</Link> with reactive field state and props

### Explanation

The `Field` component is a headless component that doesn't render any UI itself. Instead, it provides a scoped slot that receives a <Link href="../FieldStore/">`FieldStore`</Link> containing all the reactive state and props needed to render a form field.

The component automatically handles field registration, validation, and state management through the `of` prop that references the form store.

## Related

### Composables

<ApiList
  items={[
    { text: 'useForm', href: '../useForm/' },
    { text: 'useField', href: '../useField/' },
  ]}
/>

### Components

<ApiList
  items={[
    { text: 'Form', href: '../Form/' },
    { text: 'FieldArray', href: '../FieldArray/' },
  ]}
/>
