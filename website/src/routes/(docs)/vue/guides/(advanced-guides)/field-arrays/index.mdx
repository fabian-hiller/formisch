...
Now you can use the <Link href="/vue/api/insert/">`insert`</Link>, <Link href="/vue/api/move/">`move`</Link>, <Link href="/vue/api/remove/">`remove`</Link>, <Link href="/vue/api/replace/">`replace`</Link>, and <Link href="/vue/api/swap/">`swap`</Link> methods to make changes to the field array. These methods automatically take care of rearranging all the fields.
