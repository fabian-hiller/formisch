...
source: /frameworks/qwik/src/components/Form/Form.tsx
contributors:
  - fabian-hiller
---
...
The component uses the `of` prop to reference the form store and renders child elements within the form using Qwik's `<Slot />`. It also sets `noValidate` on the form element to disable browser validation, as Formisch handles validation internally.
