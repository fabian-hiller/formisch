...
Now you can use the <Link href="/preact/api/insert/">`insert`</Link>, <Link href="/preact/api/move/">`move`</Link>, <Link href="/preact/api/remove/">`remove`</Link>, <Link href="/preact/api/replace/">`replace`</Link>, and <Link href="/preact/api/swap/">`swap`</Link> methods to make changes to the field array. These methods automatically take care of rearranging all the fields.
