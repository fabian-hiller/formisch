...
Now you can use the <Link href="/svelte/api/insert/">`insert`</Link>, <Link href="/svelte/api/move/">`move`</Link>, <Link href="/svelte/api/remove/">`remove`</Link>, <Link href="/svelte/api/replace/">`replace`</Link>, and <Link href="/svelte/api/swap/">`swap`</Link> methods to make changes to the field array. These methods automatically take care of rearranging all the fields.
