...
title: Introduction
description: >-
  Formisch is a schema-based, headless form library for SolidJS. It manages
...
Formisch is a schema-based, headless form library for SolidJS. It manages form state and validation. It is type-safe, fast by default and its bundle size is small due to its modular design. Try it out in our <Link href="/playground/">playground</Link>!
...
Every form starts with the <Link href="/solid/api/createForm/">`createForm`</Link> primitive. It initializes your form's store based on the provided Valibot schema and infers its types. Next, wrap your form in the <Link href="/solid/api/Form/">`<Form />`</Link> component. It's a thin layer around the native `<form />` element that handles form validation and submission. Then, you can access the state of a field with the <Link href="/solid/api/useField/">`useField`</Link> primitive or the <Link href="/solid/api/Field/">`<Field />`</Link> component to connect your inputs.
