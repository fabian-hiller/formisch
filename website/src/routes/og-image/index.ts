...
export const onGet: RequestHandler = async ({ cacheControl, send, url }) => {
  ...
  // Fetch font data
  const [lexend400Data, lexend500Data, lexendExa600Data] = await Promise.all([
    fetchFont(fontDirUrl + '/lexend-400.ttf'),
    fetchFont(fontDirUrl + '/lexend-500.ttf'),
    fetchFont(fontDirUrl + '/lexend-exa-600.ttf'),
  ]);
  ...
  // Create Lexend Exa 600 font object
  const lexendExa600 = {
    name: 'Lexend Exa',
    data: lexendExa600Data,
    style: 'normal',
    weight: 600,
  } as const;
    ...
    send(
      new ImageResponse(
        ...
        `,
        {
          width: 1200,
          height: 630,
          fonts: [lexend400, lexend500, lexendExa600],
        }
      )
    );
        ...
        html`
          ...
          >
            <div tw="flex items-center">
              ...
              <div tw="text-8xl font-semibold text-slate-300 ml-10">
                Formisch
              </div>
            </div>
          </div>
