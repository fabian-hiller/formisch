...
export const Header = component$<HeaderProps>(({ searchOpen }) => {
  ...
  // Use root element and scrolled signal
      ...
      >
        ...
        <div class="-m-1 overflow-hidden p-1 lg:w-64">
          <Link
            ...
            onContextMenu$={() =>
              window.open(
                `${import.meta.env.PUBLIC_GITHUB_URL}/tree/main/brand`
              )
            }
          >
            <LogoIcon class="mr-1 h-7 shrink-0 md:h-8 lg:mr-2 lg:h-9" />
            <div class="font-lexend-exa truncate text-lg font-semibold md:text-xl lg:text-2xl">
        ...
        >
          {[
            { label: 'Guides', href: `/${framework.value}/guides/` },
            { label: 'API', href: `/${framework.value}/api/` },
